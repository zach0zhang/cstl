...
static void test_with_pointers()
{
	...
	for (i = 0; i <size; i++) {
		...
		printf("after push_back_cstl_vector, my_vector->elements[%d]->raw_data = %d\n", i, *(int *)elem);
		free(elem);
	}

	ret = front_cstl_vector(my_vector, &elem);
	assert(CSTL_SUCCESS == ret);
	printf("after push_back_cstl_vector, my_vector->elements[0]->raw_data = %d\n", *(int *)elem);	
	free(elem);
	
	ret = back_cstl_vector(my_vector, &elem);
	assert(CSTL_SUCCESS == ret);
	printf("after push_bacl_cstl_vector, my_vector->elements[9]->raw_data = %d\n", *(int *)elem);
	free(elem);	
